...
use tftp_rs::server::{ServerConfig, ServerEvent, run, run_on_interface};
...
#[tokio::test]
async fn interface_bound_run_rejects_a_wildcard_listener_before_resolution() {
    let dir = tempfile::tempdir().expect("temporary directory");
    let (events, _event_rx) = mpsc::unbounded_channel();
    let (_shutdown_tx, shutdown_rx) = watch::channel(false);
    ...
    let error = run_on_interface(
        SocketAddr::from(([0, 0, 0, 0], 0)),
        "not-used-after-wildcard-rejection",
        dir.path().to_path_buf(),
        events,
        shutdown_rx,
        ServerConfig::default(),
    ...
    .await
    .expect_err("wildcard binds must be rejected before interface resolution");
